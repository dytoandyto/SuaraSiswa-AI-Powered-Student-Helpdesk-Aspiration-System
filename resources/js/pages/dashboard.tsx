...
export default function Dashboard({ kategoris = [], aspirasis, stats }: any) {
    ...
    const handleFilter = (e: React.FormEvent) => {
        ...
        router.get(route('dashboard'), Object.fromEntries(formData.entries()), { preserveState: true });
    };
    ...
    // Class standar untuk input form agar konsisten dan "enak diketik"
        ...
        <AppLayout breadcrumbs={[{ title: 'Dashboard', href: '/dashboard' }]}>
            ...
            <div className="p-4 md:p-8 w-full max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 no-print border-b border-slate-200 pb-5">
                    ...
                    {isAdmin && (
                        <button onClick={() => window.print()} className="bg-slate-900 hover:bg-slate-800 transition-all text-white px-5 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 shadow-sm hover:shadow-md hover:-translate-y-0.5">
                ...
                {isAdmin ? (
                    ...
                    <div className="space-y-6">
                        {/* Filter Form Toolbar */}
                        <form onSubmit={handleFilter} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 no-print flex flex-col md:flex-row gap-3 items-center">
                            <div className="w-full md:w-1/4">
                                <input type="date" name="tanggal" className={inputClasses} />
                            </div>
                            <div className="w-full md:w-1/4">
                                <select name="kategori" className={inputClasses}>
                            ...
                            <div className="w-full md:w-2/4 flex gap-3">
                                <input type="text" name="search" placeholder="Cari NIS atau Nama..." className={inputClasses} />
                                            ...
                                            aspirasis.data.map((asp: any) => (
                                                <tr key={asp.id_aspirasi} className="hover:bg-slate-50/50 transition-colors">
                                                    <td className="px-5 py-4 align-top">
                                                        ...
                                                        <div className="font-bold text-slate-800 text-sm whitespace-nowrap">{asp.siswa?.nama}</div>
                                                        <div className="text-xs text-slate-500 font-mono mt-0.5">{asp.nis}</div>
                                                    </td>
                                                    <td className="px-5 py-4 align-top">
                                                        <div className="flex flex-wrap gap-1.5 items-center mb-1.5">
                                                            {/* PERBAIKAN LOGIKA KATEGORI MANUAL DI SINI */}
                                                            <span className="text-[9px] font-black text-indigo-700 uppercase tracking-wider bg-indigo-100/50 px-2 py-0.5 rounded border border-indigo-200">
                                                                {asp.id_kategori && asp.id_kategori !== 'manual'
                                                                    ...
                                                                    : <span className="italic">Lainnya: {asp.kategori_manual}</span>
                                                                }
                                                            </span>
                                                        </div>
                                        ...
                                        <div>
                                            ...
                                            {data.id_kategori === 'manual' && (
                                                <div className="animate-in fade-in slide-in-from-top-2 p-4 mt-3 bg-indigo-50/50 rounded-2xl border border-indigo-100">
                                                    ...
                                                    <input
                                                        type="text"
                                                        placeholder="Misal: Masalah Kantin, Ekstrakurikuler..."
                                                        ...
                                                        required
                                                    />
                                                    <p className="text-[10px] text-indigo-400 mt-2 italic">*Kategori ini akan dicatat khusus tanpa mengubah daftar kategori resmi sekolah.</p>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                                ...
                                <div className="space-y-4">
                                    {aspirasis?.data?.slice(0, 3).map((asp: any) => (
                                        <div key={asp.id_aspirasi} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all relative overflow-hidden group">
                                            {/* Garis Warna di Samping (Indikator Status) */}
